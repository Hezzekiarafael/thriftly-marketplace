...
export const seedDatabase = async () => {
  ...
  for (const productData of products) {
    ...
    if (product && product.id) {
      await productService.approveProduct(product.id, 'Approved for marketplace')
    }
  }
  ...
  if (allProducts.length >= 2) {
    await transactionService.createTransaction({
      ...
      buyerId: buyer1.id,
      hargaFinal: allProducts[0].harga,
      alamatPengiriman: 'Jl. Pemuda No. 123, Semarang',
      ongkir: 20000,
      status: 'completed'
    })
    ...
    await transactionService.createTransaction({
      ...
      sellerId: allProducts[1].sellerId,
      buyerId: buyer1.id,
      hargaFinal: allProducts[1].harga,
      alamatPengiriman: 'Jl. Pemuda No. 123, Semarang',
      ongkir: 15000,
      status: 'shipped'
    })
